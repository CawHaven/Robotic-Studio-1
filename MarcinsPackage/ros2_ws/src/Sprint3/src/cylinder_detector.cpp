...
public:
     ...
     * Initializes all components, loads the map, spawns the cylinder, and starts laser scan subscription.
     */
    CylinderDetectionNode() : Node("cylinder_detector")
    {
        // Load ground truth map 
        ground_truth_map_ = cv::imread("/home/martin/map_slam.pgm", cv::IMREAD_GRAYSCALE);
...
private:
    ...
    rclcpp::Subscription<geometry_msgs::msg::PointStamped>::SharedPtr clicked_point_sub_; /**< Subscription to the clicked point topic */
    
    // Store the last clicked point for spawning the cylinder
    std::optional<geometry_msgs::msg::Point> last_clicked_point_;
    ...
    /**
     * @brief Callback function to handle points published on the /clicked_point topic.
     * @param msg The clicked point message.
     */
    void clicked_point_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
    {
        RCLCPP_INFO(this->get_logger(), "Clicked point received at x: %.2f, y: %.2f", msg->point.x, msg->point.y);

        // Store the clicked point for spawning a cylinder
        last_clicked_point_ = msg->point;

        // Spawn the cylinder at the clicked point position
        spawn_cylinder(last_clicked_point_->x, last_clicked_point_->y);
    }
